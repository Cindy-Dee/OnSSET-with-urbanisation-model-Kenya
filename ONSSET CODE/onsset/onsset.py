...
# Additions for urbanisation reclassification
SET_ELEVATION1 = 'Elevation1'  # in metres
SET_WATER_DIST = 'WaterDist'
...
class SettlementProcessor:
    ...
    def calibrate_current_pop_and_urban(self, pop_actual, urban_current, start_year):
        """
        The function calibrates population values based on actual values provided by the user for the start year.
        Urban/rural classification is retained from the original data, after converting 1s to 0s.
        Uses population density to maintain urban/rural proportions during scaling.
        """
        logging.info('Population calibration process')
        
        # Step 1: Change values of 1 to 0 in the urban classification
        self.df[SET_URBAN].replace(1, 0, inplace=True)
        
        # Step 2: Calculate separate scaling factors for urban and rural areas
        urban_mask = self.df[SET_URBAN] > 1
        rural_mask = ~urban_mask
        
        # Calculate actual urban and rural populations based on user input
        urban_actual = urban_current * pop_actual
        rural_actual = pop_actual - urban_actual
        
        # Calculate current totals from data
        urban_current_pop = self.df.loc[urban_mask, SET_POP].sum()
        rural_current_pop = self.df.loc[rural_mask, SET_POP].sum()
        
        # Calculate separate scaling ratios
        urban_ratio = urban_actual / urban_current_pop if urban_current_pop > 0 else 0
        rural_ratio = rural_actual / rural_current_pop if rural_current_pop > 0 else 0
        
        # Step 3: Apply different scaling to urban and rural areas
        self.df[SET_POP_CALIB] = 0
        self.df.loc[urban_mask, SET_POP_CALIB] = self.df.loc[urban_mask, SET_POP] * urban_ratio
        self.df.loc[rural_mask, SET_POP_CALIB] = self.df.loc[rural_mask, SET_POP] * rural_ratio
        
        # Apply the same scaling to electrified population
        self.df[SET_ELEC_POP_CALIB] = 0
        self.df.loc[urban_mask, SET_ELEC_POP_CALIB] = self.df.loc[urban_mask, SET_ELEC_POP] * urban_ratio
        self.df.loc[rural_mask, SET_ELEC_POP_CALIB] = self.df.loc[rural_mask, SET_ELEC_POP] * rural_ratio
        
        # Calculate modeled totals
        pop_modelled = self.df[SET_POP_CALIB].sum()
        pop_urb = self.df.loc[urban_mask, SET_POP_CALIB].sum()
        urban_modelled = pop_urb / pop_actual
    
        # Store results
        self.df[SET_POP + f"{start_year}"] = self.df[SET_POP_CALIB]
        self.df['SET_URBAN' + f"{start_year}"] = self.df['IsUrban']
        
        print(f"Modelled urban population in {start_year}: {urban_modelled:.2f}")
    ...
    def max_extension_dist(self, year, time_step, end_year, start_year, grid_calc, max_intensification_cost):
        ...
        filter_lcoe, filter_investment, filter_capacity, peak_load = \
            self.get_grid_lcoe(0, 0, 0, year, time_step, end_year, grid_calc, get_max_dist=True)
            ...
            if min_dist < max_dist[i]:
                ...
                if min_dist > 0.75:
                    ...
                    for i in range(1, number_of_points + 1):
                        x_i = x + i * (x_coordinates[min_index] - x) / (number_of_points + 1)
    ...
    def elec_extension_numba(self, grid_calc, max_dist, year, start_year, end_year, time_step, grid_capacity_limit,
        ...
        while iterate:
            newly_electrified = []  # Initialize here to ensure it exists
            newly_electrified_dists = []
            new_mv_line_coords = []
            ...
            i += 1
            if len(unelectrified) == 0 or (len(newly_electrified) == 0 and len(new_electrified) > 0):
                iterate = False
        ...
        grid_lcoe, grid_investment, grid_capacity = \
            self.get_grid_lcoe(self.df['NewDist'], 0, 0, year, time_step, end_year, grid_calc)
    ...
    def calculate_new_connections(self, year, time_step, num_people_per_hh_rural, num_people_per_hh_urban):
        ...
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 0, SET_NUM_PEOPLE_PER_HH] = num_people_per_hh_rural
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 1, SET_NUM_PEOPLE_PER_HH] = num_people_per_hh_rural
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 2, SET_NUM_PEOPLE_PER_HH] = num_people_per_hh_urban
    ...
    def set_residential_demand(self, rural_tier, urban_tier, year, time_step):
        ...
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 0, SET_HH_DEMAND] = self.df[
            SET_RESIDENTIAL_TIER + str(wb_tier_rural)]
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 1, SET_HH_DEMAND] = self.df[
            SET_RESIDENTIAL_TIER + str(wb_tier_urban_clusters)]
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 2, SET_HH_DEMAND] = self.df[
    ...
    def calculate_total_demand_per_settlement(self, year, time_step):
        ...
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 0, SET_ENERGY_PER_CELL + "{}".format(year)] = \
            self.df[SET_HH_DEMAND] * self.df[SET_NEW_CONNECTIONS + "{}".format(year)]
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 1, SET_ENERGY_PER_CELL + "{}".format(year)] = \
            self.df[SET_HH_DEMAND] * self.df[SET_NEW_CONNECTIONS + "{}".format(year)]
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 2, SET_ENERGY_PER_CELL + "{}".format(year)] = \
            self.df[SET_HH_DEMAND] * self.df[SET_NEW_CONNECTIONS + "{}".format(year)]

        self.df.loc[(self.df['SET_URBAN' + f"{year - time_step}"] == 0) & (self.df[SET_ELEC_FINAL_CODE + "{}".format(
        ...
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 0, SET_TOTAL_ENERGY_PER_CELL] = \
            self.df[SET_HH_DEMAND] * self.df[SET_POP + "{}".format(year)] + produse
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 1, SET_TOTAL_ENERGY_PER_CELL] = \
            self.df[SET_HH_DEMAND] * self.df[SET_POP + "{}".format(year)] + produse
        self.df.loc[self.df['SET_URBAN' + f"{year - time_step}"] == 2, SET_TOTAL_ENERGY_PER_CELL] = \
            self.df[SET_HH_DEMAND] * self.df[SET_POP + "{}".format(year)] + produse

    def calculate_demand(self, year, num_people_per_hh_rural, num_people_per_hh_urban,
                         time_step, urban_tier, rural_tier, start_year):
        """
        ...
        """
        if year == start_year + time_step:
            self.df['SET_URBAN' + f"{start_year}"] = self.df['IsUrban'] # saving start year classification
            
        self.calculate_new_connections(year, time_step, num_people_per_hh_rural, num_people_per_hh_urban)
        self.set_residential_demand(rural_tier, urban_tier, year, time_step)
    ...
    def pv_hybrids_lcoe_lookuptable(self, year, time_step, end_year, mg_pv_hybrid_specs, pv_path=r'../test_data'):
        ...
        self.df['PVHybridGenLCOE' + "{}".format(year)] = 0.  # ToDo, needed? Also prev func
    ...
    def calculate_distance_to_urban(self, current_year, start_year, time_step):
        """
        Calculate distance to urban settlements for each grid cell using haversine formula
        """
        if current_year == start_year + time_step:
            urban_settlements = self.df[self.df['IsUrban'] > 1]
        else:
            urban_settlements = self.df[self.df['SET_URBAN' + f"{current_year - time_step}"] > 1]
        
        if len(urban_settlements) == 0:
            self.df['DistanceToUrban' + f"{current_year}"] = np.inf
            return self.df
        
        # Extract coordinates of urban settlements and all points
        urban_lons = urban_settlements[SET_X_DEG].values
        urban_lats = urban_settlements[SET_Y_DEG].values
        all_lons = self.df[SET_X_DEG].values
        all_lats = self.df[SET_Y_DEG].values
        
        # Create combined array for urban points
        urban_points = np.column_stack((urban_lons, urban_lats))
        all_points = np.column_stack((all_lons, all_lats))
        
        # Find nearest urban settlement for each point
        nearest_indices = self.do_kdtree(urban_points, all_points)
        
        # Calculate distances using haversine formula
        nearest_urban_lons = urban_lons[nearest_indices]
        nearest_urban_lats = urban_lats[nearest_indices]
        
        distances = self.haversine_vector(all_lons, all_lats, nearest_urban_lons, nearest_urban_lats)
        
        # Assign distances to dataframe
        self.df['DistanceToUrban' + f"{current_year}"] = distances
        
        return self.df
    
    def calculate_grid_dist_current(self, current_year):
        """
        Calculate GridDistCurrent_R for each grid cell using haversine formula
        """
        electrified_features = self.df[self.df[SET_ELEC_FINAL_CODE + "{}".format(current_year)].isin([1, 2])]
        
        if len(electrified_features) == 0:
            self.df['GridDistCurrent_R' + f"{current_year}"] = np.inf
            return self.df
        
        # Extract coordinates of electrified features and all points
        electrified_lons = electrified_features[SET_X_DEG].values
        electrified_lats = electrified_features[SET_Y_DEG].values
        all_lons = self.df[SET_X_DEG].values
        all_lats = self.df[SET_Y_DEG].values
        
        # Create combined array for electrified points
        electrified_points = np.column_stack((electrified_lons, electrified_lats))
        all_points = np.column_stack((all_lons, all_lats))
        
        # Find nearest electrified feature for each point
        nearest_indices = self.do_kdtree(electrified_points, all_points)
        
        # Calculate distances using haversine formula
        nearest_electrified_lons = electrified_lons[nearest_indices]
        nearest_electrified_lats = electrified_lats[nearest_indices]
        
        distances_grid = self.haversine_vector(all_lons, all_lats, nearest_electrified_lons, nearest_electrified_lats)
        
        # Assign distances to dataframe
        self.df['GridDistCurrent_R' + f"{current_year}"] = distances_grid
        
        return self.df
    
    def calibrate_logistic_urban(self, urban_ratio_start_year, urban_ratio_end_year, start_year, current_year, end_year, time_step):
        """
        Calibrate urban classification using logistic regression coefficients with feature scaling.
    
        Parameters:
        - urban_ratio_start_year: Urban population ratio at the start year
        - urban_ratio_end_year: Projected urban population ratio for the end year
        - end_year: Final year of projection
    
        Returns:
        - Modified DataFrame with urban classification
        """
            
        if f"DistanceToUrban{current_year}" not in self.df.columns:
            self.df['DistanceToUrban' + f"{current_year}"] = np.inf
        if f"GridDistCurrent_R{current_year}" not in self.df.columns:
            self.df['GridDistCurrent_R' + f"{current_year}"] = np.inf
    
        # Updated scaling parameters with precise values
        scaling_params = {
            f"ElecPopRatio{current_year}": {'mu': 0.1866, 'sigma': 0.3619},
            f"GridDistCurrent_R{current_year}": {'mu': 0.874, 'sigma': 2.7142},
            f"DistanceToUrban{current_year}": {'mu': 37.9866, 'sigma': 41.1088},
            'Elevation1': {'mu': 1.42E+03, 'sigma': 624.0154},
            'RoadDist': {'mu': 9.7461, 'sigma': 12.7298},
            f"PopDensity{current_year}": {'mu': 3.64E+03, 'sigma': 6.65E+03},
            'WaterDist': {'mu': 5.0863, 'sigma': 5.7878}
        }
    
        # Logistic regression coefficients (no intercept)
        coefficients = {
            'RoadDist': -0.20846,
            f"ElecPopRatio{current_year}": 1.2218,
            f"DistanceToUrban{current_year}": -0.45811,
            'Elevation1': -0.13544,
            f"DistanceToUrban{current_year}": -0.07062,
            f"PopDensity{current_year}": 2.6344,
            'WaterDist': -0.06802
        }
    
        # Prepare features for logistic regression
        # Calculate additional derived features
        self.df['ElecPopRatio' + f"{current_year}"] = self.df['ElecPop' + f"{current_year}"] / self.df[SET_POP + f"{current_year}"]
        self.df['PopDensity' + f"{current_year}"] = self.df[SET_POP + f"{current_year}"] / self.df['GridCellArea']
    
        # Create a copy of features for scaling
        scaled_features = self.df[[
            'RoadDist',
            f"ElecPopRatio{current_year}",
            f"DistanceToUrban{current_year}",
            'Elevation1',
            f"GridDistCurrent_R{current_year}",
            f"PopDensity{current_year}",
            'WaterDist'
        ]].copy()

        # Apply standardization scaling without zero-sigma protection
        for feature, params in scaling_params.items():
            if feature in scaled_features.columns:
                scaled_features[feature] = (scaled_features[feature] - params['mu']) / params['sigma']
            
        # Calculate linear combination (without intercept)
        linear_combination = 0  # Start with a zero value
        for feature, coef in coefficients.items():
            if feature in scaled_features.columns:
                linear_combination += scaled_features[feature] * coef  # Add each feature contribution
                
        """
        Reclassifies settlements progressively until urban population target is reached
        with early stopping when achieved
        """
        # 1. Calculate target urban population for year
        total_pop = self.df[SET_POP + f"{current_year}"].sum()
        if current_year == end_year:
            target_urban_pop = urban_ratio_end_year * total_pop
            target_ratio = urban_ratio_end_year            
        else:
            annual_increment = (urban_ratio_end_year - urban_ratio_start_year) / (end_year - start_year)
            years_elapsed = current_year - start_year
            target_ratio = urban_ratio_start_year + (annual_increment * years_elapsed)
            target_urban_pop = target_ratio * total_pop
    
        # 2. Calculate urban probabilities 
        # Convert to probability using sigmoid function
        urban_probs = 1 / (1 + np.exp(-linear_combination))

        # Save urban probabilities to dataframe for each year
        self.df['UrbanProb' + f"{current_year}"] = urban_probs

        # 3. Sort by descending probability 
        sorted_df = self.df.assign(urban_prob=urban_probs).sort_values('urban_prob', ascending=False)
        
        # 4. Progressive classification with early stopping
        urban_pop = self.df.loc[self.df[SET_URBAN] == 2, SET_POP + f"{current_year}"].sum()  # Existing urban pop
        newly_urban_pop = 0
        
        for idx, row in sorted_df.iterrows():
            if urban_pop + newly_urban_pop >= target_urban_pop:
                break  # Early exit when target reached
                
            if self.df.at[idx, SET_URBAN] != 2:  # Only reclassify non-urban
                self.df.at[idx, SET_URBAN] = 2
                newly_urban_pop += self.df.at[idx, SET_POP + f"{current_year}"]
                
                #Progress tracking
                if idx % 10000 == 0:  # Print every 10000 settlements
                    current_ratio = (urban_pop + newly_urban_pop) / total_pop
                    print(f"Progress: {current_ratio:.2%} of target {target_ratio:.2%}")
    
        # 5. Final verification
        final_urban_pop = self.df.loc[self.df[SET_URBAN] == 2, SET_POP + f"{current_year}"].sum()
        achieved_ratio = final_urban_pop / total_pop
        
        print(f"Year {current_year}: Target {target_ratio:.1%} → Achieved {achieved_ratio:.1%}")
        self.df['SET_URBAN' + f"{current_year}"] = self.df[SET_URBAN] #save classification for each year
        return self.df
        
    def project_pop_yearly_growth(self, pop_future, current_year, start_year, years_of_analysis, time_step, pop_start_year, end_year):
        """
        Projects population based on differentiated yearly growth rates:
        - Urban areas (SET_URBAN == 2): 2.5%
        - Peri-urban areas (20-35km from urban): 4.5%
        - Rural areas: 1.8%
        Scales rates to ensure total population targets are met.

        """  
        
        # --- Find the previous step population column ---
        base_pop_col = SET_POP + f"{current_year - time_step}"
    
        # --- Base growth rates ---
        urban_growth = 1.025 ** time_step  # 2.5% for urban areas
        periurban_growth = 1.045 ** time_step  # 4.5% for peri-urban (20-35km)
        rural_growth = 1.018 ** time_step  # 1.8% for rural areas

        if f"DistanceToUrban{current_year}" not in self.df.columns:
            self.df['DistanceToUrban' + f"{current_year}"] = np.inf            
            self.calculate_distance_to_urban(current_year=current_year, start_year=start_year, time_step=time_step)
            
        # --- Calculate scaling factor to meet overall target ---
        # First calculate what the total pop would be with base rates      
        urban_mask = (self.df['SET_URBAN' + f"{current_year - time_step}"] == 2)
        periurban_mask = (~urban_mask) & (self.df['DistanceToUrban' + f"{current_year}"] >= 20) & (self.df['DistanceToUrban' + f"{current_year}"] <= 35)
        rural_mask = (~urban_mask) & (~periurban_mask)
        
        # Calculate projected population with base rates
        projected_urban = self.df.loc[urban_mask, base_pop_col].sum() * urban_growth 
        projected_periurban = self.df.loc[periurban_mask, base_pop_col].sum() * periurban_growth
        projected_rural = self.df.loc[rural_mask, base_pop_col].sum() * rural_growth
        total_projected = projected_urban + projected_periurban + projected_rural
        
        # Calculate target growth for the target year
        total_growth = pop_future / self.df[base_pop_col].sum()
        step_growth_target = total_growth ** (1 / ((end_year - start_year) / time_step))
        target_pop = self.df[base_pop_col].sum() * (step_growth_target ** ((current_year - start_year) / time_step))
        
        # Calculate scaling factor to align with target
        scaling_factor = target_pop / total_projected
        
        # Apply scaled growth rates
        self.df[SET_POP + f"{current_year}"] = 0
        self.df.loc[urban_mask, SET_POP + f"{current_year}"] = (
            self.df.loc[urban_mask, base_pop_col] * urban_growth * scaling_factor
        )
        self.df.loc[periurban_mask, SET_POP + f"{current_year}"] = (
            self.df.loc[periurban_mask, base_pop_col] * periurban_growth * scaling_factor
        )
        self.df.loc[rural_mask, SET_POP + f"{current_year}"] = (
            self.df.loc[rural_mask, base_pop_col] * rural_growth * scaling_factor
        ) 

        # --- Validation ---
        total_new_pop = self.df[SET_POP + f"{current_year}"].sum()
        urban_pop = self.df.loc[urban_mask, SET_POP + f"{current_year}"].sum()
        periurban_pop = self.df.loc[periurban_mask, SET_POP + f"{current_year}"].sum()
        rural_pop = self.df.loc[rural_mask, SET_POP + f"{current_year}"].sum()
    
        print(f"Growth target for {current_year} is {step_growth_target}")
        print(f"Target Population for {current_year}: {target_pop}")
        print(f"Projected Population in {current_year}: {self.df[SET_POP + str(current_year)].sum() / 1e6:.2f} million")
        
        print(
            f"Year {current_year}: "
            f"Total Pop {current_year} = {self.df[SET_POP + str(current_year)].sum():,.0f} "
            f"(Urban: {urban_pop:,.0f}, Satellite: {periurban_pop:,.0f}, Rural: {rural_pop:,.0f})"
        )

        return self.df

    def population_projection_workflow(self, urban_ratio_start_year, urban_ratio_end_year, start_year, current_year, end_year,
                                       pop_future, years_of_analysis, time_step, pop_start_year):
        """
        Integrated workflow for urban classification and population projection
    
        Parameters:
        - urban_ratio_start_year: Urban population ratio at the start year
        - urban_ratio_end_year: Projected urban population ratio for the end year
        - current_year: Starting year of the analysis
        - end_year: Final year of projection
        - pop_future: Total projected population at the end of analysis
        - years_of_analysis: List of years to project population for
    
        Returns:
        - Modified DataFrame with projected populations
        """
        # Step 1: Calculate distance to urban settlements
        print('Calculating distance to urban for year', current_year, time.ctime())
        self.calculate_distance_to_urban(start_year=start_year, current_year=current_year, time_step=time_step)
    
        # Step 2: Calculate grid distance to current electrification
        print('Calculating distance to grid for year', current_year, time.ctime())
        self.calculate_grid_dist_current(current_year=current_year)

                # Step 4: Urban Classification
        # Classify grid cells based on urban probability
        print('Running reclassification for year', current_year, time.ctime())
        self.calibrate_logistic_urban(
            urban_ratio_start_year=urban_ratio_start_year,
        ...
        # Step 3: Population Projection for the next year of analysis
        if current_year < end_year:        
            print('Projecting population for year', current_year + time_step, time.ctime())
            self.project_pop_yearly_growth(
                pop_future=pop_future,
                start_year=start_year,
                years_of_analysis=years_of_analysis,
                time_step=time_step,
                current_year=current_year + time_step,
                pop_start_year=pop_start_year,
                end_year=end_year
            )        
        return self.df
